#!/usr/bin/env bash
# Default HTTPS Nginx Configuration
# Full SSL for all routes with HTTP to HTTPS redirect
...
create_nginx_config() {
  ...
  echo "• Creating Nginx configuration with HTTPS (full SSL)..."
...
server {
    ...
    # OCSP Stapling - Faster TLS handshakes and better privacy
    ssl_stapling on;
...
server {
    ...
    # Proxy HTTP requests to Gunicorn (NO WebSocket support)
    location / {
        proxy_pass http://gunicorn_backend;
        proxy_set_header Host $http_host;
        proxy_set_header X-Real-IP $remote_addr;
...
NGINX_CONFIG
  ...
  sed -i "s|DOMAIN_PLACEHOLDER|${DOMAIN}|g" /tmp/nginx_config
